    ...
    function loadSubscriptions(userId) {
        fetch(`${BASE_URL}/api/subscriptions/getSubscriptions.php?userId=${userId}`)
            ...
            .catch(error => {
                ...
                alert('Error loading subscriptions', false);
            });
    }
        ...
        fetch(`${BASE_URL}/api/subscriptions/addSubscription.php`, {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert(data.message || 'Failed to add subscription', false);
                }
            })
            .catch(error => {
                console.error('Error adding subscription:', error);
                alert('Error adding subscription', false);
            });
    }
            ...
            fetch(`${BASE_URL}/api/subscriptions/deleteSubscription.php`, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert(data.message || 'Failed to delete subscription', false);
                    }
                })
                .catch(error => {
                    console.error('Error deleting subscription:', error);
                    alert('Error deleting subscription', false);
                });
        }
    }
        ...
        fetch(`${BASE_URL}/api/subscriptions/modifySubscription.php`, {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert(data.message || 'Failed to modify subscription', false);
                }
            })
            .catch(error => {
                console.error('Error modifying subscription:', error);
                alert('Error modifying subscription', false);
            });
    }
    ...
    if (modifySubscriptionButton) {
        modifySubscriptionButton.addEventListener('click', function () {
            ...
            if (currentSubscriptionId) {
                ...
                alert('No current subscription to modify.', false);
            }
        });
    }
});